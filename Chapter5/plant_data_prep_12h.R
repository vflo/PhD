...
sites_list <- c(sites_list_sfd,sites_list_sf)

purrr::map(sites_list,function(x2){
  if(length(unique(x2$pl_species)) > 1){
    x2 %>% 
      split(.[['pl_species']], drop = TRUE) -> splitted
    lapply(names(splitted), function(x) {
      x2 <- splitted[[x]]
      save(x2, file=paste0('~/Chapter5/data/site_species_daylight/',
                           unique(x2$si_code),"_", x, '.RData'))
    })
  }else{save(x2, file=paste0('~/Chapter5/data/site_species_daylight/',unique(x2$si_code),"_", 
