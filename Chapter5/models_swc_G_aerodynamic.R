  ...
  if(any(colnames(x2) == "vpd_mean")
    ...
    x2%>% 
      ...
      dplyr::mutate(
        ...
        sapflow_mean = sapflow_mean*pl_sapw_area, #cm3 h-1
        ba = ((pl_dbh/2)^2)*pi,
        ba_m = ((pl_dbh/2)^2)*pi *10^(-4), #tree basal area [m2]
        st_basal_area = st_basal_area * 10^(-4), #stand basal area from m2/ha to m2 m-2
        R.a = resistance_neutral(ws_mean=ws_mean, pl_height = pl_height),
        sapflow_mean = (sapflow_mean/(60*60*10^3))/ ba_m *st_basal_area, #from cm3 h-1 to kg m-2 s-1
        cond_coeff = 1.158e2 + 4.236e-1 * ta_mean, #from Phillips and Oren
        R.t = vpd_mean/(cond_coeff*sapflow_mean), #from Phillips and Oren
        Rc = R.t-R.a,#Canopy resistance using inverted Penman-Monteith modification with asumtion -> Oren = Total see also 
        conductance = 1/Rc, # Canopy conductance, inverse of the resistance
        conductance = case_when(!is.na(si_elev) ~ conductance*44.6*(273/(273+ta_mean))*
                                  (101.325*exp(-0.00012*si_elev)/101.325),
                                is.na(si_elev) ~ conductance*44.6*(273/(273+ta_mean))),#from m s-1 to mol m-2(stand) s-1
        ...
        # pl_sapw_area = case_when(is.na(pl_sapw_area) ~ exp(log_pl_sapw_area),
        #                          !is.na(pl_sapw_area) ~ pl_sapw_area),
        pl_sapw_area = pl_sapw_area*10^(-4),
        G_sw = conductance/st_basal_area*ba_m/pl_sapw_area, #conductance per sapwood area mol m-2 s-1
        vpd_c = cond_coeff*sapflow_mean*Rc, #adapted from Meinzer 1997G_max = quantile(conductance, probs = 0.95,na.rm = TRUE),
  ...
  if (!is.null(faa) & any(!is.na(faa$swc))){
    ...
    faa2%>%
      ...
      if (
        ...
        faa2%>%
          group_by(pl_code,vpd_cut,swc_cut) %>%
          summarise(n_dist_G_cut = n_distinct(G_sw),
                    ...
                    swc_cut_mean = mean(swc_cut_mean,na.rm = TRUE),
                    ...
                    swc_shallow_true = any(is.na(swc_shallow_mean)),
                    swc_shallow_mean = mean(swc_shallow_mean,na.rm = TRUE)
...
library(optimx)
model <- lmer(G_sw~log(vpd_mean)+log(swc)+(log(vpd_mean)+log(swc)||pl_species)+(1|pl_species:pl_code),
              data=df2 )

# model <- nlme::lme((G_sw)~1,random=list(~1+log(vpd_mean)+log(swc)|pl_species,
#                                         ~1|pl_code),
#                data=df2,
#                method="ML")
