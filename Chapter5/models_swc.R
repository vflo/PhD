...
furrr::future_map(species_names,.progress=TRUE,.f=function(.x){
# purrr::map(site_names,function(.x){
  ...
  if(any(colnames(x2) == "vpd_mean")
    ...
    if((x2$pl_species == "Pinus canariensis") %>% unique()){
      t <- x2$ta_mean
      rh <- x2$rh_mean
      svp <- 610.7*10^(7.5*t/(237.3+t))
      VPD <- (1 - (rh/100))*svp/1000
      x2$vpd_mean <- VPD
      }
...
save(df2,file = "~/Chapter5/data/species_data_binned_swc.RData")    
...
save(model,file='~/Chapter5/data/model.RData')
...
save(model_log,file='~/Chapter5/data/model_log.RData')