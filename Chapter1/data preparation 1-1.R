...
subdata$actual.flow.units.corrected[subdata$actual.flow.units.corrected <= 0] <- NA
subdata$measured.flow.units.corrected[subdata$measured.flow.units.corrected <= 0 ] <- NA
subdata <- subdata[!is.na(subdata$actual.flow.units.corrected),]
subdata <- subdata[!is.na(subdata$measured.flow.units.corrected),]
real <- (subdata$actual.flow.units.corrected)
measured <- (subdata$measured.flow.units.corrected)

subdata <- cbind(subdata,real,measured)
# subdata <- subdata%>%mutate(int_range=ifelse(method!="SHB",factor(real%/%10),factor(real%/%100)))
...
Bar <- subdata %>% group_by(calibrations) %>%
  ...
  right_join(select(subdata,'study','specie',
                    'measuring.type','wood_density_in_de_article',
                    'density','correc.aplication.2', 'year', 'DBH',
                    'vessel_area', 'vessel_density',
                    'porosity','MF','RC','TC','time.resolution',
                    'n.gauges', 'calibrated', 'calibrations','calibrations_pairs',
                    'calibrations_calibrated','calibrations_corrected',
                    'method','correction','new_methods','CORRECT_APLICATION',
...
#    dplyr::summarize(NRMSE=sqrt(sum(dife1)/n()))%>%    
#    right_join(Bar,by = 'calibrations')
# 
# #RRMSE
# Bar <- subdata %>% group_by(calibrations) %>%
...
#   right_join(subdata[,c('study','specie','measuring.type','wood_density_in_de_article','density','correc.aplication.2', 'year', 'DBH','vessel_area', 'vessel_density',
...
#   mutate(logratio = log(measured/real)) %>%
#   dplyr::summarize(ratio.var = ((sd(logratio))^2)) %>%
#   right_join(Bar[,c('study','specie','measuring.type','wood_density_in_de_article','density','correc.aplication.2', 'year', 'DBH','vessel_area', 'vessel_density',
#                     'porosity', 'dens_cat','MF','RC','TC','time.resolution', 'calibrated', 'calibrations','calibrations_pairs',
#                     'calibrations_calibrated','calibrations_corrected','method','correction','new_methods','CORRECT_APLICATION')], 
#              by = 'calibrations') %>%
#   unique() %>%
